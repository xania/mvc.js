...
export default function actionResolver<TAction>(
    input: ActionResolverInput<TAction>
): ActionResolver<TAction> {
    ...
    if (Array.isArray(input)) {
        ...
        return function resolve(
            ...
            for (var i = 0; i < mappings.length; i++) {
                ...
                if (match) {
                    const actionContext: IActionContext = createContext(
                        parentContext,
                        match.appliedRoute,
                        match.params
                    );
    ...
    function mappingFromInput(mapping: RouteMappingInput<TAction>) {
        if (Array.isArray(mapping)) {
            const resolver =
                typeof mapping[0] === "function"
                    ? mapping[0]
                    : routeResolver(mapping[0]);
...
function fromObject<TAction>(
    mappings: RouteMappingObject<TAction>
): ActionResolver<TAction> {
    if (mappings) {
