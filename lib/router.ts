...
export class Router {
    ...
    start<TAction, TActionResult extends ActionResult>(
        ...
        viewEngine: ViewEngine<TAction, TActionResult>
        ...
        function resolveRoute(
            ...
            route: Route
        ): Rx.Observable<ActionResolution<TAction>> {
        ...
        function expandRoute(
            routeEntry: RouteEntry<TAction, TActionResult>,
            routeIndex: number
        ): Rx.Observable<RouteEntry<TAction, TActionResult>> {
            ...
            return resolveRoute(resolver, remainingRoute).pipe(
                Ro.mergeMap((resolution: ActionResolution<TAction>) => {
                    ...
                    const nextRemainingRoute = remainingRoute.slice(
                        appliedRoute.length
                    );
                    if (nextRemainingRoute.length === 0) {
                        routeCache.truncateAt(routeIndex + 1);
                    }
                    const url: UrlHelper = new UrlHelper(
                        router,
                        appliedRoute,
                        routeEntry.actionContext.url
                    );

                    const actionContext: IActionContext = {
                        url,
                        params: {
                            ...routeEntry.actionContext.params,
                            ...resolution.params,
                        },
                    };
                    ...
                    try {
                        let nextResult$ = viewEngine.execute(
                            resolution.action,
                            actionContext
                        );

                        return toObservable(nextResult$).pipe(
                            Ro.map((nextResult) => {
                                const nextRouteResult = routeResult(
                                    appliedRoute,
                                    nextResult
                                );
                                routeCache.setAt(routeIndex, nextRouteResult);
                                const nextRouteEntry: RouteEntry<
                                    TAction,
                                    TActionResult
                                > = {
                                    action,
        ...
        return router.actions$.pipe(
            Ro.map((route) =>
                startsWith(route, router.baseRoute)
                    ? route.slice(router.baseRoute.length)
                    : []
            ),
            ...
            Ro.switchMap((remainingRoute) => {
                ...
                return Rx.of(rootEntry).pipe(
                    ...
                    Ro.filter(
                        (entry) =>
                            entry.remainingRoute.length ===
                            0 /* is last entry */
                    ),
                    Ro.map((entry) =>
                        router.activate(
                            entry.routeResult.actionResult,
                            entry.actionContext.url.toAbsolute()
                        )
                    )
                );
            }),
...
function routeCompare(xv: any[], yv: any[]) {
    ...
    while (i--) {
        ...
        if (pathCompare(prev, next) === false) return false;
    }
...
export function toObservable<T>(
    input: T | Rx.SubscribableOrPromise<T>
): Rx.Observable<T> {
...
function routeResult<TActionResult>(
    appliedRoute: Route,
    actionResult: TActionResult
): RouteResult<TActionResult> {
    return {
        appliedRoute,
        actionResult,
    };
...
export interface ViewEngine<TAction, TActionResult> {
    ...
    ): TActionResult | Rx.SubscribableOrPromise<TActionResult>;
    ...
    resolve(
        ...
        error: Error,
        route: Route,
        context: RouteEntry<TAction, TActionResult>
    );
}
