...
export class Router {
    ...
    start<TAction, TActionResult extends ActionResult>(
        ...
        function resolveRoute(
            ...
            return toObservable(
                resolver && resolver.call(null, route, context)
            );
        }
        ...
        function expandRoute(
            ...
            return resolveRoute(resolver, remainingRoute, parentContext).pipe(
        ...
        const rootContext: IActionContext = {
            url: new UrlHelper(this, this.baseRoute, 0),
            params: {},
        };
        ...
        return router.actions$.pipe(
            ...
            Ro.switchMap((remainingRoute) => {
                ...
                return Rx.of(rootEntry).pipe(
                    ...
                    Ro.filter(
                        (entry) => {
                            return entry.remainingRoute.length === 0;
                        } /* is last entry */
                    ),
        ...
        function activate(
            entry: RouteEntry<TAction, TActionResult>
        ): Activation {
...
export function toObservable<T>(
    input: T | Rx.Subscribable<T> | PromiseLike<T>
): Rx.Observable<T> {
...
export interface IViewEngine<TAction, TComponent> {
    execute(
        action: TAction,
        context: IActionContext
    ): TComponent | SubscribableOrPromise<TComponent>;
...
export class Activation {
    constructor(
        public subscriptions: Rx.Unsubscribable[],
        public route: Route
    ) {}
